# coding=utf-8
'''
generate covariance matrices that follow a block structure and are recognized by the limix package as covariance matrices that can be optimized
'''


#### PACKAGES ####
import numpy as np
from limix_core.covar import SumCov, FixedCov, DiagonalCov
##################
...
def covar_block(dim, block_dims, id=False):
	...
	for i in range(numBlocks):
		...
		if i == 0:
			Cov = SumCov(FixedCov(Cov_i))
		else:
			Cov = SumCov(Cov, FixedCov(Cov_i))
		...
		if numBlocks > 1 and i > 0:
			for j in range(i):
				Cov_i_edge = covar_block_side(dim,
											  ...
											  start2 = start[i-j-1],
											  end2 = end[i-j-1])
				Cov = SumCov(Cov, FixedCov(Cov_i_edge))

		start.append(start[i] + dims[i])
		
	if id:
		Cov = SumCov(Cov, FixedCov(np.eye(dim)))
	
	return Cov
	
	
	
def covar_block_single(dim, start, end, id=True):
...
def covar_block_side(dim, start1, end1, start2, end2):
	...
	cov[start1:end1, start2:end2] = 1e-8 * np.ones((end1 - start1, end2 - start2))
	#make symmetric
	cov[start2:end2, start1:end1] = 1e-8 * np.ones((end2 - start2, end1 - start1))
